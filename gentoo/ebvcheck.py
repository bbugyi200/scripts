...
def check_pkg(  # noqa: C901
    ...
    try:
        ...
        latest_version_A = get_latest_version(pkg)
        if latest_version_A is None:
            ...
            if latest_version_A != version:
                latest_version_B = latest_version_A
                latest_version_A = get_latest_version(
                    pkg, key=lambda x: -fuzzy_match(pkg, x), use_cache=True
                )
            ...
            if latest_version_A != version:
                ...
                if latest_version_A == latest_version_B:
                    ...
                    new_version = f"{latest_version_B} OR {latest_version_A}"
...
def get_latest_version(
    full_pkg: str, key: Callable = len, use_cache: bool = True
) -> Optional[str]:
    import requests
    ...
    if use_cache and hasattr(self, pkg_attr):
    ...
    for pkg_key in sorted(data, key=key):
        version_set = {
            D['version'] for D in data[pkg_key] if '.' in D['version']
        }
