...
def new_torrent_worker(args: argparse.Namespace) -> None:
    ...
    thread = threading.Thread(target=tw.work, daemon=True)
    thread.start()
        ...
        try:
            ...
            while True:
                ...
                try:
                    sp.check_call(
                        lib.DELUGE +
                        ["add", "-p", self.download_dir, self.magnet]
                    )
                except sp.CalledProcessError:
                    i += 1
                    if i <= 10:
            ...
            lib.notify_and_log(
                'Finished Downloading "{}".'.format(self.title)
            )
            ...
            with lib.the_plague:
                ...
                kill_worker(self.magnet_tracker[mkey])
